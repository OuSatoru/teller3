...
def onKeyboardEvent(event):
    if event.Key == 'Rshift':
        ...
        time.sleep(0.7)
        print(lines[a][4])
        #SendMessage(ID_hwnd, win32con.WM_SETTEXT, None, lines[a][4])
        time.sleep(0.03)
        PostMessage(ID_hwnd, win32con.WM_KEYDOWN, win32con.VK_RETURN, None)
        time.sleep(0.1)
        try:
            cant = FindWindow('输入身份证号码')
            qd = FindWindowEx(cant, None, 'Button', None)
            SendMessage(qd, win32con.BM_CLICK, 0, -1)
            SendMessage(tID_hwnd, win32con.WM_SETTEXT, None, lines[a][4])
            SendMessage(tName_hwnd, win32con.WM_SETTEXT, None, lines[a][2].strip())
            SendMessage(tAdd_hwnd, win32con.BM_CLICK, 0, -1)
        finally:
            pass
        time.sleep(0.2)
        SendMessage(print_hwnd, win32con.BM_CLICK, 0, -1)
